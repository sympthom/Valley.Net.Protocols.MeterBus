...
/// <summary>
/// Integration tests requiring a real M-Bus TCP collector.
...
{
    private const string CollectorIpAddress = "192.168.1.66";
    ...
    public async Task Meter_Should_Respond_With_Ack_When_Sending_SND_NKE()
    {
        ...
        var endpoint = new TcpBinding(new IPEndPoint(IPAddress.Parse(CollectorIpAddress), CollectorPort), new MeterbusFrameSerializer());
    ...
    {
        var binding = new TcpBinding(new IPEndPoint(IPAddress.Parse(CollectorIpAddress), CollectorPort), new MeterbusFrameSerializer());
    ...
    public async Task Meter_Scanner_Should_Find_Meter_When_Connected()
    {
        ...
        await master.Scan([0x0a], TimeSpan.FromSeconds(100));
    }
    ...
    public async Task Meter_Telemetry_Should_Be_Retrieved_When_Querying()
    {
        ...
        var response = await master.RequestData(0x0a, Timeout) as VariableDataPacket;
